...
class PPO(object):
    def __init__(self, model, replay_buf, lr=3e-4,
    ...
    def compute_gae(self, next_state, lammbda=0.95):
        ...
        # _, _, value = self.model.select_action(torch.FloatTensor(next_state[None]).to(device))
        with torch.no_grad():
            _, _, value = self.model.select_action(next_state)
